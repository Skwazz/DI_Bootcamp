...
const getCarHonda = (carInventory) => {
  const findHonda = carInventory.find((car) => car.car_make == `Honda`);
  console.log(
    `This is a ${findHonda.car_make} ${findHonda.car_model} from ${findHonda.car_year}`
  );
};
...
const sortCarInventoryByYear = (carInventory) => {
  const sortYear = carInventory.sort((a, b) => {
